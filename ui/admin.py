...
from .models import (
    # Asosiy
    Category, Instructor, Course, Section, Lesson,
    # Quiz
    Quiz, Question, Choice, QuizAttempt,
    # Torrens
    TorrensTest, TorrensTask, TorrensSubmission, TorrensAnswer,
    # Boshqa
    LibraryItem, HomepageSlide, Testimonial
)
...
class CategoryAdmin(admin.ModelAdmin):
    ...
    # Admin formda slug maydoni ko'rinsin va JS bilan nomdan to'ldirilsin:
...
class LessonInline(admin.TabularInline):
    model = Lesson
    extra = 0
    fields = ("order", "kind", "title", "video_url", "video_file",
              "document_file", "duration_minutes")
...
class CourseAdmin(admin.ModelAdmin):
    list_display = ("cover", "title", "category", "instructor", "price", "is_featured", "created_at")
    list_filter = ("category", "is_featured")
    search_fields = ("title", "instructor__full_name")

    # Slug add formida ko‘rinadi va sarlavhadan to‘ldiriladi
    prepopulated_fields = {"slug": ("title",)}
    fields = (
        "category", "instructor",
        "title", "slug", "short_desc", "image",
        "price", "rating", "duration_hours",
        "students_count", "is_featured",
        "created_at",
    )
...
class QuestionAdmin(admin.ModelAdmin):
    list_display = ("text_short", "quiz", "qtype", "points", "order", "choices_count")
...
class QuizAdmin(admin.ModelAdmin):
    list_display = (
        "__str__", "lesson_link", "num_questions",
        "limit_questions", "shuffle_questions", "shuffle_choices",
        "pass_percent", "time_limit_seconds", "attempts_limit",
    )
    list_editable = ("limit_questions", "shuffle_questions", "shuffle_choices", "pass_percent")
...
class QuizInline(admin.StackedInline):
    ...
    fields = (
        ...
        "pass_percent", "time_limit_seconds", "attempts_limit",
        "limit_questions", "shuffle_questions", "shuffle_choices",
        "advice_low", "advice_mid", "advice_high",
        "advice_mid_min", "advice_high_min",
    )


@admin.register(Lesson)
class LessonAdmin(admin.ModelAdmin):
    list_display = ("title", "section", "kind", "order", "duration_minutes", "has_quiz", "manage_quiz")
...
class QuizAttemptAdmin(admin.ModelAdmin):
    list_display = ("user", "quiz", "lesson", "percent", "points_gained", "points_total", "created_at")
    ...
    def export_attempts_csv(self, request, queryset):
        ...
        w = csv.writer(resp)
        w.writerow([
            "User","Quiz ID","Lesson ID",
            "Points gained","Points total","Percent",
            "Created","Finished","Question IDs","Answers JSON"
        ])
        for a in queryset:
            w.writerow([
                str(a.user) if a.user_id else "",
                a.quiz_id, a.lesson_id,
                a.points_gained, a.points_total, a.percent,
...
if _HAS_MULTI_IMAGES:
    ...
    class TorrensTaskImageInline(admin.TabularInline):
        ...
        extra = 0
        # Eslatma: models.py da 'order' maydoni yo‘q — shuning uchun ko‘rsatmaymiz
...
class TorrensTaskInline(admin.TabularInline):
    ...
    # allow_* maydonlari models.py da bo'lmasa ham xato bermasligi uchun dinamik fields:
...
class TorrensTestAdmin(admin.ModelAdmin):
    ...
    prepopulated_fields = {"slug": ("title",)}
    fields = ("title", "slug", "description", "is_published", "time_limit_minutes", "created_at")
    readonly_fields = ("created_at",)
...
class TorrensSubmissionAdmin(admin.ModelAdmin):
    ...
    def export_torrens_csv(self, request, queryset):
        ...
        for s in queryset:
            w.writerow([str(s.user), s.test_id, s.status, s.total_score, s.started_at, s.finished_at])
        return resp
...
class TorrensAnswerAdmin(admin.ModelAdmin):
    ...
    def drawing_thumb(self, obj):
        if obj.drawing_image:
            return format_html('<img src="{}" style="height:40px;border-radius:4px" />', obj.drawing_image.url)
        return "—"
...
class LibraryItemAdmin(admin.ModelAdmin):
    list_display = ("cover", "title", "standard", "display_creator", "display_year", "is_published", "created_at")
    list_filter = ("standard", "is_published")
    search_fields = ("title", "dc_creator", "dc_format")
    readonly_fields = ("created_at",)
    fieldsets = (
        ("Umumiy", {"fields": ("is_published", "standard", "title", "file", "link", "thumb")}),
        ("Dublin Core (DC)", {
            "classes": ("collapse",),
    ...
    def cover(self, obj):
        if getattr(obj, "thumb", None):
            return format_html('<img src="{}" style="height:36px;border-radius:6px" />', obj.thumb.url)
        return "—"
...
class TestimonialAdmin(admin.ModelAdmin):
    list_display = ("full_name", "role", "rating", "is_published", "order", "created_at")
    list_filter = ("is_published",)
